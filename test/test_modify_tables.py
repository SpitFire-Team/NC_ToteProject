...
@pytest.fixture
def tables_1(sales_order_df):
    tables_1 = [{"sales_order": sales_order_df}]
    return tables_1

@pytest.fixture
def tables_2(sales_order_df, staff_df):
    tables_2 = [{"staff": staff_df},{"sales_order": sales_order_df}]
    return tables_2

class TestDataframeModification:

    def test_dataframe_modification_returns_list_of_dicts(self, tables_1, tables_2):
        """
        This tests that the function returns a list of dictionaries as the return value.

        """
        assert type(dataframe_modification(tables_1)) == list
        assert type(dataframe_modification(tables_2)) == list
        assert type(dataframe_modification(tables_2)[0]) == dict


    def test_datafram_modification_has_correct_table_keys(self, staff_df, sales_order_df):
        """
        This tests that the returned dictionaries have the correct table names as a key.
        """
        test_dict_list = [{"sales_order": sales_order_df}]
        test_staff_list = [{"staff": staff_df}]
    ...
    def test_datafram_modification_has_correct_table_keys_for_multiple_items(self, tables_2):
        """
        ...
        """
        table_names = []
        for item in dataframe_modification(tables_2):
    ...
    def test_dataframe_moficiation_removes_create_at_column(self, sales_order_df, tables_1):
        """
        This tests that the returned dataframes have the "created_at" column removed.
        """
        assert "created_at" in list(sales_order_df.columns.values)
        result = dataframe_modification(tables_1)
        assert "created_at" not in list(result[0]["sales_order"].columns.values)


    def test_dataframe_modification_removes_created_at_column_from_all_items_in_multi_item_list(self, tables_2):
        """
        This tests that the returned dataframes have the "created_at" column removed
        when there are multiple dictionaries in the input list.
        """
        for item in tables_2:
            target_dataframe = list(item.values())[0]
            assert "created_at" in list(target_dataframe.columns.values)
        result = dataframe_modification(tables_2)
        for item in result:
            target_dataframe = list(item.values())[0]
            assert "created_at" not in list(target_dataframe.columns.values)


    def test_dataframe_moficiation_removes_last_updated_column(self, sales_order_df, tables_1):
        """
        This tests that the returned dataframes have the "last_updated" column removed.
        """
        assert "last_updated" in list(sales_order_df.columns.values)
        result = dataframe_modification(tables_1)
        assert "last_updated" not in list(result[0]["sales_order"].columns.values)


    def test_dataframe_modification_removes_last_updated_column_from_all_items_in_multi_item_list(self, staff_df, sales_order_df):
        """
        This tests that the returned dataframes have the "last_updated" column removed
        when there are multiple dictionaries in the input list.
        """
        test_dict_list = [{"sales_order": sales_order_df}, {"staff": staff_df}]

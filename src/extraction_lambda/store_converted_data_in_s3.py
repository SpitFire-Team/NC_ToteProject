...
from src.utils.aws_utils import add_json_to_s3_bucket, make_s3_client, get_bucket_name
from src.utils.file_utils import convert_to_dict, get_path_date_time_string, db_values
...
def input_updated_data_into_s3(s3_client, db_updated_values):  # db_values will be a list of dictionaries (JSON)
    '''
        convert db_updated_values in json format seperated by table. uploads to table folder in the s3 bucket
                Parameters:
                        s3_client (boto3.client('s3')): a boto3 client for an s3 bucket
                        db_updated_values ({(<updated_table_name>, column_list) : all updated date since last call})
                Returns:
                        none
    '''
    ...
    for table_name, transformed_values in transformed_data.items():
        ...
        add_json_to_s3_bucket(s3_client=s3_client, bucket_name=bucket_name, data=json_compatable_data, file_path=file_path)
